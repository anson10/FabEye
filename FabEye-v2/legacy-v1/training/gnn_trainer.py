...
class GNNTrainer:
    ...
    def train_epoch(self, loader) -> dict:
        self.model.train()
        total_loss = sev_loss_sum = type_loss_sum = 0.0
        correct = total = 0

        for batch in loader:
            x, edge_index, b, y_type, y_sev = self._move_batch(batch)

            self.optimizer.zero_grad()
            type_logits, severity = self.model(x, edge_index, b)
            loss, breakdown = self.criterion(type_logits, severity, y_type, y_sev)
            loss.backward()
    ...
    def validate_epoch(self, loader) -> dict:
        ...
        for batch in loader:
            x, edge_index, b, y_type, y_sev = self._move_batch(batch)
            type_logits, severity = self.model(x, edge_index, b)
            loss, _ = self.criterion(type_logits, severity, y_type, y_sev)
            total_loss += loss.item() * y_type.size(0)
            metrics.update(type_logits, severity, y_type, y_sev)
